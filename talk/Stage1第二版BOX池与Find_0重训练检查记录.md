...
## 2026-08-11 00:35：累计检查确认 Find_0 完成第十四次验证

- 18:34之后的例行检查确认 Find_0 Job 336298 attempt a4先健康推进到step20381；没有发现需要即时记录的异常。到本次检查时，训练已推进到 `trainer/global_step=20564` 并完成第十四次验证。验证总损失为 `0.2967804`；配体体素、受体、实际原子和伪原子PRAUC分别为 `0.6028082`、`0.6723844`、`0.6658018` 和 `0.6250064`，均未超过各自现有最佳值。
- 当前总损失、原子损失、受体损失、配体体素损失和伪原子损失分别为 `0.2576109`、`0.1287554`、`0.1320577`、`0.0989446` 和 `0.1670509`，均为有限值。新的 `TOP_epoch_00_score_0.6028.ckpt` 与 `last.ckpt` 已完整落盘；`BEST.ckpt` 已刷新为当前全局最佳 `TOP_epoch_00_score_0.6105.ckpt`。top-k保留数量维持10份，最弱的 `TOP_epoch_00_score_0.5527.ckpt` 已按策略移除。
- 两张H100使用80,960/81,559 MiB和80,780/81,559 MiB，利用率为85%和100%。计算节点的正式进程继续使用release `Pocket_Plus_dc0f05168815`及batch6/global48、warmup0.005、patience3、在线W&B契约。Slurm状态为 `RUNNING`，只有 `after_lock_336298`；W&B摘要和内部日志持续更新，计算节点有43个匹配正式训练命令的进程，当前OOM、Traceback和NCCL错误扫描为空。

## 2026-08-11 09:36：累计检查确认 Find_0 完成第十五次验证

- 00:35之后的两轮例行检查确认 Find_0 Job 336298 attempt a4依次健康推进到step21170和21803；没有发现需要即时记录的异常。到本次检查时，训练已推进到 `trainer/global_step=22004` 并完成第十五次验证。验证总损失由 `0.2967804` 降至新低 `0.2904713`；受体、实际原子和伪原子PRAUC刷新为新高 `0.6874556`、`0.6870933` 和 `0.6447831`，配体体素PRAUC为 `0.5916340`，未超过当前最佳 `0.6105047`。
- 当前总损失、原子损失、受体损失、配体体素损失和伪原子损失分别为 `0.3026987`、`0.1436599`、`0.1474068`、`0.1211573` 和 `0.2314076`，均为有限值。新的 `TOP_epoch_00_score_0.5916.ckpt` 与 `last.ckpt` 已完整落盘；`BEST.ckpt` 继续指向当前全局最佳 `TOP_epoch_00_score_0.6105.ckpt`。top-k保留数量维持10份，最弱的 `TOP_epoch_00_score_0.5607.ckpt` 已按策略移除。
- 两张H100使用80,764/81,559 MiB和80,982/81,559 MiB，利用率为73%和55%。计算节点的正式进程继续使用release `Pocket_Plus_dc0f05168815`及batch6/global48、warmup0.005、patience3、在线W&B契约。Slurm状态为 `RUNNING`，只有 `after_lock_336298`；W&B摘要和内部日志持续更新，计算节点有43个匹配正式训练命令的进程，当前OOM、Traceback和NCCL错误扫描为空。

## 2026-08-11 19:47：累计检查确认 Find_0 完成第十六次验证

- 09:36之后的两轮例行检查确认 Find_0 Job 336298 attempt a4依次健康推进到step22931和23381；没有发现需要即时记录的异常。到本次检查时，训练已推进到 `trainer/global_step=23651` 并完成第十六次验证。验证总损失由 `0.2904713` 降至新低 `0.2841025`；配体体素、受体、实际原子和伪原子PRAUC均刷新为新高 `0.6130216`、`0.6975111`、`0.7026834` 和 `0.6617029`。
- 当前总损失、原子损失、受体损失、配体体素损失和伪原子损失分别为 `0.2585915`、`0.1262849`、`0.1298248`、`0.1001573` 和 `0.1916681`，均为有限值。新的 `TOP_epoch_00_score_0.6130.ckpt` 与 `last.ckpt` 已完整落盘；`BEST.ckpt` 按一次验证延迟继续指向上一轮全局最佳 `TOP_epoch_00_score_0.6105.ckpt`。top-k保留数量维持10份，最弱的 `TOP_epoch_00_score_0.5624.ckpt` 已按策略移除。
- 两张H100使用80,634/81,559 MiB和80,928/81,559 MiB，利用率为97%和37%。计算节点的正式进程继续使用release `Pocket_Plus_dc0f05168815`及batch6/global48、warmup0.005、patience3、在线W&B契约。Slurm状态为 `RUNNING`，只有 `after_lock_336298`；W&B摘要和内部日志持续更新，计算节点有43个匹配正式训练命令的进程，当前OOM、Traceback和NCCL错误扫描为空。

## 2026-08-12 10:43：累计检查确认 Find_0 完成第十七次验证

- 19:47之后的两轮例行检查确认 Find_0 Job 336298 attempt a4依次健康推进到step24212和24779，没有发现需要即时记录的异常。本次检查时训练已推进到 `trainer/global_step=26084`，并确认第十七次验证已经完成。验证总损失由 `0.2841025` 降至新低 `0.2831193`；配体体素、受体和实际原子PRAUC刷新为新高 `0.6190767`、`0.6998192` 和 `0.7032412`，伪原子PRAUC为 `0.6567278`，未超过现有最佳 `0.6617029`。
- 当前总损失、原子损失、受体损失、配体体素损失和伪原子损失分别为 `0.2334691`、`0.1105580`、`0.1198872`、`0.0928868` 和 `0.1803560`，均为有限值。新的 `TOP_epoch_00_score_0.6191.ckpt` 与 `last.ckpt` 已完整落盘；`BEST.ckpt` 按一次验证延迟刷新到上一轮全局最佳 `TOP_epoch_00_score_0.6130.ckpt`。top-k保留数量维持10份，最弱的 `TOP_epoch_00_score_0.5668.ckpt` 已按策略移除。
- 两张H100分别使用80,804/81,559 MiB和80,710/81,559 MiB，即约78.91/78.82 GiB。正式进程继续使用release `Pocket_Plus_dc0f05168815`及既定训练参数；Slurm状态为 `RUNNING`，只有 `after_lock_336298`，W&B持续更新，计算节点有43个匹配正式训练命令的进程，当前OOM、Traceback和NCCL错误扫描为空。

## 2026-08-12 16:44：Find_0 完成第十八次验证

- Find_0 Job 336298 attempt a4推进到 `trainer/global_step=26780` 并完成第十八次验证。配体体素与实际原子PRAUC由 `0.6190767` 和 `0.7032412` 提升到新高 `0.6202929` 和 `0.7065769`；验证总损失为 `0.2840298`，受体与伪原子PRAUC为 `0.6995640` 和 `0.6497418`，未超过各自现有最佳 `0.2831193`、`0.6998192` 和 `0.6617029`。
- 当前总损失、原子损失、受体损失、配体体素损失和伪原子损失分别为 `0.2547380`、`0.1114086`、`0.1159317`、`0.1110213` 和 `0.2071486`，均为有限值。新的 `TOP_epoch_00_score_0.6203.ckpt` 与 `last.ckpt` 已完整落盘；`BEST.ckpt` 按一次验证延迟刷新到上一轮全局最佳 `TOP_epoch_00_score_0.6191.ckpt`。top-k保留数量维持10份，最弱的 `TOP_epoch_00_score_0.5769.ckpt` 已按策略移除。
- 两张H100分别使用80,836/81,559 MiB和80,710/81,559 MiB，即约78.94/78.82 GiB，利用率为75%和100%。正式进程继续使用release `Pocket_Plus_dc0f05168815`，Slurm状态为 `RUNNING`，只有 `after_lock_336298`，W&B摘要持续更新。allocation错误文件自attempt a4启动时没有变化，其中的Traceback属于attempt a1缺少 `.project-root` 的历史错误；当前没有OOM、NCCL错误或未处理异常。

## 2026-08-12 19:45：hnode01 意外重启，Job336298 自动重新排队
