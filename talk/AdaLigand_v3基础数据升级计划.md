...
> 状态：名单初始化与配体类别掩码已经正式运行并验收；配体语言模型管线已完成本地实现和本地验证，正式 CPU、GPU 与汇总任务尚未提交，提交权归用户。
...
和自编码器池化；截断前后 token 事实都写入结果。SMI-TED 的独立运行时固定
...
完整字段、续跑规则和命令见
`Data_Preprocessing/Ori_Data_upgrade/ligand_language_model/README.md`。

## 12. 计划执行顺序

代码实现和服务器执行分别授权。阶段 A、阶段 B 已正式运行并验收；阶段 D 已完成
本地实现和本地验证，但正式服务器命令仍由用户提交。
...
3. MoLFormer 与 SMI-TED 分别运行；每个 GPU 数组任务跨其负责的全部 PDB occurrence 组成全局 batch，再把向量分发回逐 PDB 目录。
4. MoLFormer 默认 batch size 256；SMI-TED 使用官方 `model.encode` 和 batch size 100，当前分片少于 100 条时使用实际条数。
5. 本地验证字段、精确对应、后备图、续跑、普通 batch 逐条重试和最终全局汇总；临时校验代码不进入正式目录。
...
阶段 A 与阶段 B 已由用户提交并验收。阶段 D 已完成本地代码、临时测试和服务器
命令准备；AI 不提交正式配体语言模型任务、不写正式服务器产物，除非用户再次
明确授权。阶段 C 与阶段 E 仍不得开始实现。
