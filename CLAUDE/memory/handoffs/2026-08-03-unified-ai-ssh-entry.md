...
Windows 用户级统一入口已经建立，AdaLigand 的项目入口只负责把参数转交给它。项目文件和交接记录均不保存服务器密码。

统一入口为 `C:\Users\15919\.codex\tools\Invoke-ProjectSsh.ps1`，AdaLigand 入口为 `与服务器交互/other/Invoke-PasswordSsh.ps1`。AI agent 可以通过 `-Command` 执行单条远端命令，也可以通过 `-Command "bash -s" -InputFile <LF 脚本>` 执行本地脚本。
...
- AdaLigand 的旧入口改为薄包装，不再要求调用者显式提供密码文件。
