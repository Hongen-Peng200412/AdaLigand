<#
.SYNOPSIS
    Clean sync AdaLigand Ori_Data code to the remote server.

.DESCRIPTION
    Human-only destructive sync:
    1. Delete the remote AdaLigand Ori_Data directory.
    2. Verify rsync exists on the remote server.
    3. Recreate the remote directory.
...
Write-Host " CLEAN SYNC: this deletes the remote target before upload." -ForegroundColor Red
Write-Host " Use manually only. Agents must not run this script." -ForegroundColor Red
Write-Host "==========================================================" -ForegroundColor Red

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host " Starting Clean Sync to $RemoteIP (Port $RemotePort)..." -ForegroundColor Cyan
...
Write-Host "[1/6] Cleaning remote directory..." -ForegroundColor Yellow
$DeleteCmd = "if [ -d '$RemoteFullDir' ]; then rm -rf '$RemoteFullDir' && echo 'Remote directory deleted successfully.'; else echo 'Directory does not exist, skipping deletion.'; fi"
Invoke-RemoteCommand $DeleteCmd

if ($LASTEXITCODE -ne 0) {
    Write-Host "Warning: potential issue during remote cleaning. (Exit Code: $LASTEXITCODE)" -ForegroundColor Yellow
}

Write-Host "[2/6] Checking remote rsync..." -ForegroundColor Yellow
...
Write-Host "[3/6] Recreating remote directory..." -ForegroundColor Yellow
$CreateDirCmd = "mkdir -p '$RemoteFullDir'"
Invoke-RemoteCommand $CreateDirCmd

if ($LASTEXITCODE -ne 0) {
    Write-Host "Error recreating remote directory." -ForegroundColor Red
    exit $LASTEXITCODE
}
