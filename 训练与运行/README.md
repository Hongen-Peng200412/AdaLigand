...
单一数据任务的脚本放入对应任务目录的 `sh/`；只有项目级入口放入 `训练与运行/sh/`。例如，服务器的 AdaLigand 项目根目录中存在 `训练与运行/sh/prepare_labels.sh` 时运行：
...
单一数据任务的脚本放在对应任务目录的 `sh/`，项目级入口放在 `训练与运行/sh/`；两者都必须位于任务根目录内。最小示例：
...
通常只需新增或修改职责所属目录 `sh/` 中的人类入口脚本；只有项目级入口放在 `训练与运行/sh/`。资源申请留在 `submit_task.sh` 参数中；项目环境、Python 入口、配置文件和结果目录写在任务脚本中。不要把某次作业编号、某台节点或某个临时目录写进通用运行组件。
...
迁移不会自动改写旧脚本，也不会删除 CPC、数据处理或推理任务的既有能力。需要复用旧任务时，应先把任务主体整理为职责所属目录 `sh/` 中可冷读的入口；只有跨任务的项目级入口放入 `训练与运行/sh/`，随后仍通过本目录提交。
