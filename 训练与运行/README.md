...
本目录提供一套可以随项目复制的 Slurm 任务提交系统。读者只需要准备一个 Bash 任务脚本，就可以申请计算资源、冻结本次执行所见的完整项目、保留启动证据，并通过四种锁文件控制同一份计算资源内的重复执行和退出。
...
Slurm 作业可能排队数小时。如果在提交命令运行时就复制代码，作业真正开始时可能仍执行旧代码。本系统在每次删除 `try_lock`、即将再次执行任务时才计算项目内容哈希并创建 release，因此该次执行与对应 release 一一对应。
...
可以显式提供 `--partition`、`--qos`、`--nodelist`、`--mem`、`--time`、`--nodes` 和 `--array`。显式值优先于硬件类型默认值。

## 8. 四种锁文件如何控制任务

- `pre_lock_<job-id>`：仅使用 `--hold` 时创建。删除后才开始第一次执行。
- `try_lock_<job-id>`：每次执行结束后创建。可以先编辑 `run_cmd_<job-id>.sh`，再删除它以复用当前资源执行下一次命令。
...
`--simple` 保留四锁，但不创建 release 和 launch，任务直接读取当前项目目录。控制文件和 Slurm 日志统一放在 `$HOME/SIMPLE_RUN`。
