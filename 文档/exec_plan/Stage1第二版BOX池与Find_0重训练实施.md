...
- [x] (2026-08-10 03:28+08:00) 服务器连接已恢复。只读检查确认 Find_0 attempt a4 推进到 `global_step=17537`，五项训练损失有限；第十一次验证、TOP0.5900、last与当前全局最佳TOP0.5946的 `BEST.ckpt` 保持完整，尚无第十二次验证。正式训练进程继续使用release `Pocket_Plus_dc0f05168815`、batch6/global48、warmup0.005、patience3与在线W&B。两张H100使用80.14/79.88 GiB，作业为 `RUNNING` 且只有 `after_lock_336298`，当前错误扫描为空。
- [x] (2026-08-10 06:30+08:00) Find_0 attempt a4推进到 `global_step=17804` 并完成第十二次验证。验证总损失降至 `0.295210`，配体体素、受体、原子和伪原子PRAUC均刷新为新高 `0.601202`、`0.683164`、`0.683900` 和 `0.636159`。新TOP0.6012与last完整，`BEST.ckpt`仍按一次验证延迟对应上一轮最佳TOP0.5946；top-k保留策略移除最弱TOP0.5116。两张H100使用79.02/78.55 GiB，作业为 `RUNNING` 且只有 `after_lock_336298`，当前错误扫描为空。
...
- [x] (2026-08-11 00:35+08:00) 累计检查确认 Find_0 attempt a4先健康推进至step20381，随后在 `global_step=20564` 完成第十四次验证。验证总损失为 `0.296780`，配体体素、受体、原子和伪原子PRAUC为 `0.602808`、`0.672384`、`0.665802` 和 `0.625006`，未超过现有配体体素最佳0.610505。新TOP0.6028与last完整，`BEST.ckpt`刷新到全局最佳TOP0.6105；top-k保留策略移除最弱TOP0.5527。两张H100使用79.06/78.89 GiB，作业为 `RUNNING` 且只有 `after_lock_336298`，W&B持续更新，无OOM或未处理异常。
- [x] (2026-08-11 09:36+08:00) 累计检查确认 Find_0 attempt a4在两轮例行检查中健康推进至step21803，随后在 `global_step=22004` 完成第十五次验证。验证总损失刷新为新低 `0.290471`；受体、原子和伪原子PRAUC刷新为新高 `0.687456`、`0.687093` 和 `0.644783`，配体体素PRAUC为 `0.591634`，未超过现有最佳0.610505。新TOP0.5916与last完整，`BEST.ckpt`继续指向全局最佳TOP0.6105；top-k保留策略移除最弱TOP0.5607。两张H100使用78.87/79.08 GiB，作业为 `RUNNING` 且只有 `after_lock_336298`，W&B持续更新，无OOM或未处理异常。
- [x] (2026-08-11 19:47+08:00) 累计检查确认 Find_0 attempt a4在两轮例行检查中健康推进至step23381，随后在 `global_step=23651` 完成第十六次验证。验证总损失刷新为新低 `0.284102`；配体体素、受体、原子和伪原子PRAUC均刷新为新高 `0.613022`、`0.697511`、`0.702683` 和 `0.661703`。新TOP0.6130与last完整，`BEST.ckpt`按一次验证延迟继续指向上一轮全局最佳TOP0.6105；top-k保留策略移除最弱TOP0.5624。两张H100使用78.74/79.03 GiB，作业为 `RUNNING` 且只有 `after_lock_336298`，W&B持续更新，无OOM或未处理异常。
