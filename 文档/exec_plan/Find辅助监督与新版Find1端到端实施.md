...
- [x] (2026-08-24 02:55--03:04+08:00) Job `351295` 的第 2 次执行完成第二次正式 validation，并从 `global_step=2255` 恢复训练到 `global_step=2282`。验证总损失从 `0.424685` 降至 `0.364538`；配体体素、受体、蛋白主链、核酸主链、原子和伪原子 PR-AUC 分别为 `0.552578`、`0.615148`、`0.689030`、`0.000117`、`0.626517` 和 `0.598371`。配体体素 PR-AUC 比首次验证提高约 `0.040460`，超过调度器绝对改善阈值 `0.003`；W&B 摘要中的 `warmup_lr` 为 `3.3454e-5`，本轮没有平台期降学习率。`TOP_epoch_00_score_0.5526.ckpt` 与 `last.ckpt` 已完整落盘，大小均为 1,466,223,480 字节；`BEST.ckpt` 已刷新为上一轮的 `TOP_epoch_00_score_0.5121.ckpt`，当前最高 TOP 按冻结回调语义等待后续 validation 刷新别名。03:04 的两张 H100 快照利用率为 69% 和 78%，显存占用约 78.95/81.56 GiB 与 80.79/81.56 GiB；唯一控制锁仍为 `after_lock_351295`，错误扫描为空。
- [x] (2026-08-24 08:53--08:59+08:00) Job `351295` 的第 2 次执行完成第三次正式 validation，并从 `global_step=3386` 恢复训练到 `global_step=3404`。验证总损失继续降至 `0.341108`；配体体素、受体、蛋白主链、核酸主链、原子和伪原子 PR-AUC 分别为 `0.561822`、`0.636451`、`0.958468`、`0.000172`、`0.650053` 和 `0.608337`。配体体素 PR-AUC 比第二次验证提高约 `0.009244`，再次超过绝对改善阈值 `0.003`；W&B 摘要中的 `warmup_lr` 为 `4.1757e-5`，本轮没有平台期降学习率。`TOP_epoch_00_score_0.5618.ckpt` 与 `last.ckpt` 已完整落盘，大小均为 1,466,223,991 字节；`BEST.ckpt` 已刷新为上一轮的 `TOP_epoch_00_score_0.5526.ckpt`，当前最高 TOP 仍按冻结回调语义等待后续 validation 刷新别名。唯一控制锁仍为 `after_lock_351295`，错误扫描为空；验证后步数继续推进。
- [x] (2026-08-24 14:49--14:56+08:00) Job `351295` 的第 2 次执行完成第四次正式 validation，并从 `global_step=4514` 恢复训练到 `global_step=4535`。验证总损失继续降至 `0.321514`；配体体素、受体、蛋白主链、核酸主链、原子和伪原子 PR-AUC 分别为 `0.576911`、`0.639860`、`0.983063`、`0.880060`、`0.664888` 和 `0.582926`。配体体素 PR-AUC 比第三次验证提高约 `0.015089`，再次超过绝对改善阈值 `0.003`；`warmup_lr` 已达到配置的最高学习率 `5.0e-5`，本轮没有平台期降学习率。`TOP_epoch_00_score_0.5769.ckpt` 与 `last.ckpt` 已完整落盘，大小均为 1,466,224,438 字节；`BEST.ckpt` 已刷新为上一轮的 `TOP_epoch_00_score_0.5618.ckpt`，当前最高 TOP 仍按冻结回调语义等待后续 validation 刷新别名。唯一控制锁仍为 `after_lock_351295`，错误扫描为空；验证后步数继续推进。
...
Revision note 2026-08-24 03:04+08:00：记录 Job `351295` 第 2 次执行的第二次正式 validation、六项 PR-AUC、新 TOP/last checkpoint、`BEST.ckpt` 的一轮延迟刷新状态，以及验证后恢复训练且当前错误扫描为空的证据。

Revision note 2026-08-24 08:59+08:00：记录 Job `351295` 第 2 次执行的第三次正式 validation、六项 PR-AUC、新 TOP/last checkpoint、`BEST.ckpt` 的一轮延迟刷新状态，以及验证后步数继续推进且当前错误扫描为空的证据。

Revision note 2026-08-24 14:56+08:00：记录 Job `351295` 第 2 次执行的第四次正式 validation、六项 PR-AUC、新 TOP/last checkpoint、warmup 达到 `5.0e-5`、`BEST.ckpt` 的一轮延迟刷新状态，以及验证后步数继续推进且当前错误扫描为空的证据。
