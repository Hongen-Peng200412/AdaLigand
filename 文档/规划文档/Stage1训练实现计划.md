...
|---|---|---|---|
| `unet_c1` | 从头 | voxel ligand + voxel auxiliary | validation voxel ligand PR-AUC |
| `Find_0/CPC1` | 从头 | voxel ligand、voxel auxiliary、A、P | validation voxel ligand PR-AUC |
| `Find_0/CPC2` | 自己的 CPC1 BEST | A、P；体素分支冻结 | validation P PR-AUC |
| `Find_1/CPC1` | 从头 | voxel ligand、voxel auxiliary、A、P | validation voxel ligand PR-AUC |
| `Find_1/CPC2` | 自己的 CPC1 BEST | A、P；体素分支冻结 | validation P PR-AUC |
...
以下四项是并列、显式、可独立验收的数据准备，不得藏在训练循环或 `Dataset.__getitem__` 中：

1. **Stage1 input eligibility**：从 Stage G 最终 keep-list 扫描完整网格；任一轴 `<80` 的 PDB–EMDB pair 进入排除清单，原因固定为 `grid_axis_lt_80`。它是 Stage1 不合格，不得让训练崩溃，也不得在 batch 内临时跳过。
2. **主 split**：只对 eligible pair 划分 train/validation/calibration/held-out pool。
3. **训练 BOX 池与固定 validation selection**：按本文规则预计算轻量起点。
4. **held-out pool 去冗余**：论文严格测试前再做；它与前三项同等独立，但不阻塞当前训练、calibration 或近期实验。
...
每项都应有自己的入口、冻结清单、版本、输入身份和统计。训练启动只读取这些产物，不现场生成或修补。
...
split 在看见模型结果前一次冻结。calibration 不参与 epoch/checkpoint 选择；validation 不选择完整图阈值；当前不把未去冗余 held-out pool 冒充测试集。

### 2.3 统一 80³ 起点解析

所有训练、验证、滑窗和居中请求最终都调用同一个起点解析器。完整网格形状为 `full_shape_zyx=(Lz,Ly,Lx)`，请求起点为 `requested_start_zyx`，BOX 边长固定 80：
...
eligibility 已保证 `L_a≥80`，所以所有 crop 都是完整、真实的 80³ 网格，不做图外补零，不产生空间 `voxel_valid_mask`。向内平移后，原目标不必继续严格位于 BOX 中心。
...
context 直接复用项目已有的一键生成逻辑：每个合法 context 必须包含至少 1000 个 core receptor heavy atoms。context 不绑定 occurrence。
...
- context 从该 PDB context 池选 3 个。

训练不再施加在线平移 jitter。只保留现有同步随机 90° 空间旋转，并同时作用于密度、受体坐标、hardmask 和全部 target；仅 train 启用。validation、calibration 和所有推理关闭旋转。
...
### 3.3 receptor 的 10 Å 加载、8 Å模型视图与 core scatter

统一 Dataset/Collator 以 80³ core BOX 加 10 Å buffer 加载全部 receptor atoms，并保留：

- `atom_global_indices`；
- 世界 XYZ 与 BOX 局部坐标；
- `atom_is_in_core_box`；
- 到 core BOX 的几何距离或等价的 8 Å model-view mask。

两个 Find 实际点云分支只消费 core+8 Å 的 receptor model view。多加载的外侧 2 Å 不进入 Find 前向，但使 BOX 级原始 receptor 契约统一，并允许下游按全局索引恢复 10 Å 原始上下文。
...
| `hardmask` | `[80,80,80] bool` | core receptor 的唯一 home-voxel 集合；只用于 auxiliary 监督/诊断 |
...
| 字段 | shape / dtype | 语义 |
|---|---|---|
| `atom_global_indices` | `[N_A] int64` | 10 Å加载集合在整图 receptor 表中的索引 |
...
| `atom_is_in_find_view` | `[N_A] bool` | core+8 Å点云 model view |
| `atom_label` | `[N_A] bool` | 仅 train/validation；binding label |

Collator 堆叠 dense voxel 字段；变长原子表按“拼接 + `atom_batch_index` + offsets/counts”组织，不 pad 到固定原子数。推理时不伪造全零 targets。
...
### 4.2 Find_0：无 embed head 的硬 scatter 基线

`Find_0` 不实例化 embed head；不得把 `passthrough` 模块冒充成学习后的 embed 输出。模型语义为：

- point backbone 直接消费 8 Å model view 中的 raw 49D receptor features；
...
`Find_0` 的配置可以用 `embed_head: null` 或等价的不构建路径；验收标准是参数树中不存在 embed head 参数，point 输入确为 49D，voxel scatter 确为上述硬 scatter。
...
8 Å model view 由 Dataset/model-view mask 决定，不靠上述 block radius 实现。非块式编码固定为：
...
4. point value：128D 表示投影到 64D，与 `Linear(49→64)` raw residual 相加，residual gate 固定为 1。
5. voxel backbone 输入为 56D density + 51D receptor = **107 channels**；point backbone 输入为 64D。

residual、occupancy、centroid encoding、MLP 和 soft splat 被允许；Transformer block 明确不允许。若现有 `Stage1EmbedHead` 把非块式投影和 block 构造耦合，最小修改应是允许 block 数为 0 后仍执行这些投影，不另造第二个 embed subsystem。
...
不得再生成或读取 `ligand_dist_map`，不得用 1.7 Å 阈值派生 ligand target。hardmask 不得乘到 ligand 概率、ligand target 或完整图输出上。
...
- `unet_c1`、`Find_0/CPC1`、`Find_1/CPC1`：最大化 `val_score/global/voxel_ligand_PRAUC`。
- `Find_0/CPC2`、`Find_1/CPC2`：最大化 `val_score/global/pseudo_PRAUC`。

同时记录 voxel auxiliary AP、A binding AP、各 loss、Dice 和运行诊断，但不拼成综合 BEST 分数。固定 validation 只选 checkpoint；三个 producer 的完整图方案比较与阈值选择在 calibration 100 上进行，见并列推理计划。
...
它与完整 forward 使用同一 embed/scatter、voxel input 构建和固定 3 次 voxel recycle；不生成 P candidates，不执行 point backbone、A/P heads 或 sparse-refine，也不导出居中特征。当前模型不存在 A/P 后半段回写 voxel 分支的路径，因此该入口必须与完整 forward 的 `voxel_logits_ligand` 逐元素等价，而不是近似模型。
...
2. **eligibility**：任一轴 `<80` 的样本在训练前被稳定排除；正式 Dataset 不临时 skip。
3. **统一路径**：四种请求 provider 经过同一个 materializer/collator；推理不构造 fake target。
4. **10/8/core 三层 receptor**：Dataset 加载 10 Å、Find point view 为 8 Å、voxel scatter 仅 core；全局索引不丢失。
5. **Find_0**：无 embed 参数，hard floor/sum scatter 49D，voxel input 105D，point input 49D。
6. **Find_1**：三类 block 都为 0，非块式 MLP/residual/centroid/soft-splat 生效，voxel input 107D，point input 64D。
...
通过后可以并行启动 `Find_0/CPC1`、`Find_1/CPC1` 与 `unet_c1`；每个 Find 的 CPC2 只等待自己的 CPC1 BEST。无需等待另一个 Find 或 unet 完成。三者 BEST 冻结后，立即按 `Stage1训练与多阈值推理.md` 对 calibration 100 进行完整图推理、阈值标定与指标汇报。
...
- 不在训练循环中现场划 split、生成 BOX 池、过滤小图或修 target。
- 不引入在线平移 jitter、图外 padding、空间 `voxel_valid_mask` 或候选相关训练样本。
- 不让 Find_1 的 embed 前处理出现 Transformer block；不把 8 Å model view 写成 block radius。
- 不让 `unet_c1` 读取 sim/diff/receptor 特征，也不取消其真实 auxiliary head。
- 不让 Find voxel scatter 看见 buffer 原子；不把额外加载的 10 Å 全部送入 point backbone。
