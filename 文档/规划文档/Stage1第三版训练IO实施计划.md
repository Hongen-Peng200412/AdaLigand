...
本文是第三版 split、BOX pool 和迁移后 NPY 的训练消费主规格。统一 Dataset/Loader、模型输入接缝和训练入口已经收口；2026-08-24 的修订保持 V3 几何候选池不变，把活动训练与验证请求改为 PDB 中心采样。历史 Job `346737` 仍按当时的请求规则解释：该单卡主链版 `unet_c1` 于 2026-08-23 正常完成，最佳 validation 配体体素 PRAUC 为 `0.602950`。新版推理算法属于独立时间节点。
...
- V3 逐 PDB BOX 候选池保持不变。活动训练使用 `pdb_foreground_box_num=25`、`pdb_foreground_fraction_target=0.5` 与 `pdb_occurrence_foreground_box_cap=5`；验证冻结相同规则的 epoch 0 到 `validation_selection_pdb_centric.npz`。原 `validation_selection.npz` 与 `config.json::entry_ratio` 只保留历史构建含义。第二版数据目录保留，不读取、不改写、不删除。
...
- 训练请求按 manifest 中的 PDB 顺序生成。一个含 `O` 个 occurrence 的 PDB 每个 epoch 使用 `min(25, 5O)` 个 bias；bias 尽可能均匀分配到全部 occurrence，不能整除的余数按 epoch 轮转。context 固定选择 25 个，不因 bias 不足而减少。
...
- 当前 PDB 中心采样入口统一使用 `max_epochs=70` 和 `warmup_ratio=0.005`。Find_0 每个 epoch 执行 9 次完整 validation；Find_1 与三个 U-Net 入口每个 epoch执行 11 次。该组合使总训练 BOX 数、相邻验证之间的训练 BOX 数和 warmup 期间的 BOX 数尽可能接近旧请求规则。Stage1 CPC1 与 CPC2 的 `ReduceLROnPlateau` 都采用绝对改善阈值 `0.003`；该阈值不修改 Selector 的独立优化参数。
