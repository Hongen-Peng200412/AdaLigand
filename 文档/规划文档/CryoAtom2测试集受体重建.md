...
三个 Slurm 数组任务各申请一张 A100 和 8 核 CPU，以测试清单中 `pdb_ids[分片编号::3]` 分配 PDB。使用项目已有提交系统，保留 `--after_hold` 行为，不传 `--time`。失败 PDB 记录原因后继续同分片其他 PDB；不换回真实受体、不静默缩小测试集。已验收成功的产物可以跳过，失败重试必须保留先前运行证据。
