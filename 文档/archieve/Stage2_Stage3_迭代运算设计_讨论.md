...
> **Stage3 Phase 1 权威提示（2026-08-07）**：本文的 Stage2 算子和监督说明继续有效；本文涉及独立 `Stage3Builder`、共享表示前导和密度分支的旧设计不进入当前 Phase 1。密度模块之前的 Stage3 唯一实施规格是 `文档/规划文档/Stage3_PocketXMol_Phase1适配与复现.md`：先严格适配并复现 PocketXMol 四种 docking 组合，完成前不实现密度或微调。本文旧 Stage3 内容只保留为 Phase 1 后重新讨论的候选设计。
