...
| **3** | per-BOX 添油 | per-BOX，按 `(box_type, box_id)` keyed | Stage1 推理后 / 任意时刻 | **只增不改** | blob（mask + 对各 in-box occurrence 的覆盖 + provenance）、ligand-area 内 UNet 特征、未来任意派生特征 |
...
| **49 维受体特征** | `(N_rec, 49) float32` | 受体 token + 在线特征工程 | **必须整图算**（见 §2.1） |
| 受体几何/索引 | coords、element、res_type、atom_name、res_index、chain_index | Stage C3 `receptor_tokens.npz` | 全局世界坐标 |
...
> `centroid_atom` 是 C4 既有产物（`coords`+`present`）的纯派生，可现场算；是否物化进 `ligand_coords.npz` 留作便利项，不强制。
...
### 4.2 blob（Stage1 box 级推理产出，喂 Stage2 的关键料）

**每个 box 至多一个 blob**，充当覆盖矩阵的一行：

- **box 级推理**：一个冻结 box 跑 Stage1，可能出多个 blob → 取 **mask 体素中心离 box 中心最近**的那一个，其余忽略（邻近 occurrence 由它们自己的 box 负责）。
- **全图划窗推理（infered_box）**：先全图推理，再对每个 blob **以它的中心定一个 box** → 天生一框一 blob，不涉挑选。
...
| 字段 | 形式 | 说明 |
|---|---|---|
| `blob_mask` | ragged 体素 mask（COO/offset） | 已过 binding 概率硬过滤（见 `迭代运算` pipeline 约定） |
| `blob_voxels` | int | `|b|`，供覆盖派生 |
| `prompt_point` | `[3] float` | blob 的 ligand-area 体素质心 → 交 Stage3 |
| `provenance` | dict | Stage1 checkpoint、阈值、硬过滤参数 |

### 4.3 coverage（稀疏，落 blob 时顺手算）

落 blob 的同时，算这个 blob 对**各 in-box occurrence**（§3.2）的覆盖，**稀疏落盘**（只存有交的条目）：
...
### 4.4 ligand-area 内 UNet 特征（按 box、完整不截断）

| 字段 | 形式 | 说明 |
|---|---|---|
| `la_voxel_index` | ragged `[K,3]` | **预测 ligand-area 之内**的体素索引，**完整存**（K 可变、不截断） |
| `la_voxel_feat` | ragged `[K, C_total]` | 两源堆叠：Stage1 UNet `voxel_final_feat` ⊕ 预训练化学 UNet 特征 |

- PP 采样运行时从这里 top-k（加 min 兜住太小的 blob）；**灌不灌背景是一个运行时开关，不在落盘层决定**。
- Stage1 UNet 很重、**不能现场算**，故必须落；**只落 ligand-area 内**把体量从"整图几十 TB"压到"单位数 TB"。同一区域的 center/bias box 因 Stage1 特征带 box 上下文而**各不相同**，不是冗余副本，是有意的增广。
...
| 档 | 用户输入 | 激活的块 | 备注 |
|---|---|---|---|
| **纯密度档** | 只有原始密度图 | 第 1 层 density（+56 通道）、第 2 层、第 3 层 blob/coverage/UNet 特征 | 前置：Stage1 也只吃密度 |
| **纯受体档** | 只有受体结构 | 第 1 层 49 维受体特征 + 几何、第 2 层 `occ_distances`（距离-O） | **无密度、无 blob**；可独立训练 |
| **统一档** | 密度 + 受体 | 上面两者全要 | 受体块**加到密度上** |

可拆卸 = 丢掉受体块退回纯密度、丢掉密度块退回纯受体，**消融和换档是同一个开关**。
...
| **Stage2（Match）** | 第 2 层 + 第 3 层 blob/coverage | 同 PDB 多 box 的 blob 拼成行、图级 occurrence 拼成列（§6）；混采 bias（真）+ context（假）+ infered_box（自然）；口袋以 **blob 为锚**（§8.2） |
...
| **infered_box** | 全图划窗推理：一个 blob 以其中心定一框 | 由覆盖/距离事后给出 | 自然分布（merge/split、真实假阳） |
...
4. `centroid_atom` 是否物化进 `ligand_coords.npz`（默认派生）。
