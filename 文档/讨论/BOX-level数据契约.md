...
>
> **统一锚（贯穿全文）**：所有 BOX 共用同一套轻量身份 schema。`center/bias/context` 只表示训练 BOX 原点来源；blob 只来自全图推理并只属于 `infered_box`。一个 `clg_group_parent` 角色的 `infered_box` 对应一个 CLG，框内可以有多个 Global Proposal Candidate；多阈值路线交给 Stage2/3 的正式 proposal 是反链选择结果，F1 baseline 则直接交付全部合法 F1 blob，二者都不是把 BOX 本身当作 blob。局部重跑的 mask 只作辅助观察，不能替换全图候选身份。
...
| **3** | per-BOX 添油 | 仅需要添油的 BOX，按 `addon_run_id + BOX key` keyed | Stage1 整图推理后 / 任意时刻 | **只增不改** | proposal/CLG 表、Group-parent 共享 voxel/P/receptor 表、候选 voxel/receptor membership、GT overlap、selector score、selection result、可选 refinement 与密度 U-Net 特征 |
...
```json
{
  ...
  "box_start_zyx": [z0, y0, x0],     // 相对完整网格的整数起点；可以为负或越过网格末端
  ...
  "source_tree_id": -1,
  "source_node_id": -1,
  "occ_distances": [                  // 稀疏：BOX 中心到各 in-box occurrence 的描述性距离，见 §3.2
    {"cid": 3, "dist": 1.2},
    {"cid": 5, "dist": 31.7}
  ]
}
...
- **`box_id` 在 `(pdb_id, box_type)` 内唯一且跨 run 不复用**：每个 box_type 有自己的持久分配器；后续 proposal run 只能追加新的未用 ID。BOX key 为 `(pdb_id,box_type,box_id)`，第 3 层再加 `addon_run_id` 区分同一 BOX 的不同模型/配置产物（§4.1）。
- `box_start_zyx: int32[3]` 使全图 voxel index 与 BOX 局部 index 能精确互换。它不做 in-grid clamp：居中 BOX 可以以负数开始，也可以在任一轴越过完整网格末端。`origin_xyz: float32[3]` 严格定义为 BOX 局部 voxel `[0,0,0]` 的**中心**世界坐标，并按下式从可能越界的起点线性外推；底层文件若使用 voxel 外角，数据适配层必须先转换。
...
切片契约固定为：先求 BOX 与完整网格的矩形交集，把交集内容放到对应局部索引，其余位置常数补零。`exp`、`sim`、GT mask 与概率图都按此规则补零；网格外不存在 receptor 原子。有效矩形完全可由 `box_start_zyx`、`shape_zyx` 和 `full_grid_shape_zyx` 确定性恢复，盘上不保存 `voxel_valid_mask`。
...
- 用途仅是 BOX 级空间检索、采样诊断和训练 BOX 的描述性距离；它不圈定 overlap 或覆盖矩阵的列，也不充当 infered proposal 的候选级距离标签。

> **没有 `main_occurrence_id`、没有 `member_occurrences`**：覆盖矩阵的列是**图级全部 occurrence**（与 box 无关）；proposal overlap 由 `Global Proposal Mask × occurrence GT mask` 决定，候选级距离由 proposal mask 质心与 occurrence 原子质心决定——都不由 `occ_distances` 的稀疏范围限制。
...
- 全量描述子拼成一张 **`box_index`** 表（`pdb_id, box_type, box_id, box_start_zyx, origin_xyz, shape_zyx, materialization_role, proposal_run_id, clg_id, source_tree_id, source_node_id, ...`）。addon inventory 不属于冻结描述子，由 §4.1 的独立索引维护。
...
addons/{addon_name}/{addon_run_id}/{pdb_id}.npz
  内部: (box_type, box_id) → entry 的映射；变长内容用 offsets + indices 存
```

- **加新特征 = 新建 `addon_name/addon_run_id` namespace**，老添油一字节不动。`addon_run_id` 是生产 checkpoint、配置、输入 manifest 与 schema version 的联合稳定身份；proposal、selector、selection 和 refinement 分别使用 `proposal_run_id`、`selector_run_id`、`selection_run_id`、`refinement_run_id`。
- 完整寻址键是 **`(addon_name,addon_run_id,pdb_id,box_type,box_id)`**。同一 NPZ 已由目录确定前三项，文件内部只需保存 `(box_type,box_id)` entry 映射。
- 独立 `addon_inventory` 只增不改地记录上述完整键、文件引用和内容哈希；不得回写冻结的 `box_index` 描述子。
- 第一版 NumPy 物理产物统一用 `np.savez_compressed` 写入。若 IO benchmark 后改用 Zarr 或其它容器，解码后的字段、dtype、shape、offsets/indices 语义和压缩要求不得变化。
...
| `proposal_config` | JSON | 体素上下界、统一边界规则、split/merge 深度、`N_abs`、候选上限等完整配置 |
...
稳定 ID 作用域固定为：`tree_id` 在 `(proposal_run_id,pdb_id)` 内唯一，`node_id` 在 `tree_id` 内唯一，`clg_id` 在 `(proposal_run_id,pdb_id)` 内唯一，`candidate_index` 在 `clg_id` 内连续且稳定，`box_id` 在 `(pdb_id,box_type)` 内唯一；`selector_run_id` 与 `selection_run_id` 分别由各自 manifest 定义。跨 addon 引用必须携带足以确定作用域的上级 ID，不得依赖文件 entry 顺序。
...
| 字段 | shape / 逻辑 dtype | 说明 |
|---|---|---|
| `clg_id` | scalar `int32` | `(proposal_run_id,pdb_id)` 内唯一 |
...
- `candidate_voxel_indices` 是 Global Proposal Mask，决定 selector 标签、打分和 Stage2/3 默认 blob。同一候选内部必须唯一且排序；不同候选可以引用相同 parent index，交叉是合法的。
...
语义 F1 阈值层的每个合法连通 blob 都建立一个居中的 `infered_box`，设置 `materialization_role=f1_baseline`，并由 `proposal_run_id + pdb_id + source_tree_id + source_node_id` 唯一指向原始全图 F1 节点。F1 baseline 保存全部合法 F1 blob，不受多阈值路线 `N_abs` 限制。
...
对每个全图 proposal blob 计算它与同一 PDB 各 GT occurrence mask 的非零交集。多阈值路线中，一条 proposal entry 对应 CLG 的一个 `candidate_index`；F1 baseline 中，一个 BOX 只有它自己的 F1 proposal entry。所有量都以原始全图 mask 为准，不使用局部重跑形状或 Refined Mask。

设 `N_proposal` 为本 addon 中的 proposal 数，`E` 为非零 proposal-occurrence 记录数：

| 字段 | shape / 逻辑 dtype | 说明 |
|---|---|---|
| `proposal_box_id` | `[N_proposal] int32` | 每个 proposal 所属 `infered_box` |
...
proposal 的完整稳定 join key 是 `(proposal_run_id,pdb_id,proposal_tree_id,proposal_node_id)`；前两项来自本 addon 的 `addon_run_id`/manifest，不能只用 tree/node 跨 run 连接。`proposal_box_id` 负责回到同一 PDB 的冻结 BOX 描述子。
...
由上述 overlap 与第 1 层 GT mask 体积还可派生 selector 软标签 `q_i=max_j IoU(GlobalProposal_i,GT_j)`、最佳 occurrence，以及一对一匹配所需的 IoU 矩阵。oracle 最优反链、`y_CLG` 与空集标签由 selector 训练配置中的 `lambda_count` 派生并单独缓存，不写入不可变的基础 overlap addon。
...
| `P_feat_L2` | `[N_P,C_{P2}] float16` | density-box 特征 |
| `P_feat_L3` | `[N_P,C_{P3}] float16` | cross-attention 前的 P 特征 |
| `P_feat_L4` | `[N_P,C_{P4}] float16` | cross-attention 后的 P 特征 |
...
| `receptor_feat_L1` | `[R,C_{R1}] float16` | receptor embed head 后特征 |
| `receptor_feat_L2` | `[R,C_{R2}] float16` | receptor density-box 特征 |
| `receptor_feat_L3` | `[R,C_{R3}] float16` | receptor cross-attention 前特征 |
| `receptor_feat_L4` | `[R,C_{R4}] float16` | receptor cross-attention 后特征 |
...
受体坐标、49 维特征、元素/残基和键表由 `receptor_atom_global_index` 回到第 1 层获取，不重复保存。同一候选内部 receptor indices 唯一且排序，不同候选可以共享原子；交叉是合法的。空 P 由 `N_P=0` 表示，空 receptor 子集由相邻 offsets 相等表示；不在磁盘写伪 token。

#### 4.4.3 feature-source manifest

所有 `voxel_feat__*`、`P_feat_L2/L3/L4`、`receptor_feat_L1/L2/L3/L4` 都必须有来源清单：
...
Stage2/3 的 `FuseSources` 从这些稳定来源装配输入。上述 P/receptor 层名及其语义是核心 schema；feature-source manifest 用来补充生产模块出口、checkpoint、通道数、dtype 与坐标帧，不能用一个泛化 `source_name` 取代层次语义。
...
直接得到。`n_pocket_atoms`、Rg、残基直方图、Wiener index 和 graph energy 均可由候选 receptor membership 与第 1 层 receptor 表确定性派生，第一版不物化这些化学/碎片描述子，以减少契约复杂度。
...
| `selector_manifest_ref` | string + hash | 含 `lambda_count`、one-to-one oracle 与 tie-break、三项 loss 权重、`gamma_focal`、概率编码和模型配置 |
...
| `selection_logit` | `[N] float32` | 反链可加分数中的逐候选 logit `z_i` |

#### 4.7.2 selection addon

| 字段 | shape / 逻辑 dtype | 说明 |
|---|---|---|
| `selection_run_id` | string | selector score、原始树 DP 与解码配置的联合身份 |
| `selector_run_id` | string | 使用哪次候选打分 |
| `clg_id` | scalar `int32` | CLG |
| `selected_candidate_indices` | `[N_selected] int16` | 原始组件树上精确 MAP 的反链；空集为正式结果，允许长度为 0 |
| `selection_score` | scalar `float32` | `sum(selection_logit[selected_candidate_indices])`；空集固定为 0 |
| `empty_set_probability` | scalar `float32` | 同一原始树 DP 的结构化 softmax 空集概率 |

反链 log-partition、MAP 和空集概率都在当前 CLG 候选 `G` 于完整原始组件树上的最小连接闭包中精确 DP；只有 `i∈G` 的节点可被选择，闭包内其它节点只传递结构状态。不得把同一原树中其它 CLG 的候选纳入本样本，也不落盘 Candidate-induced Tree。MAP 同分时先选更小反链，再按排序后的原始 `node_id` 字典序裁决；该规则属于 selection run identity。训练 oracle 使用的 `lambda_count` 属于标签缓存和 selector manifest；改变它必须重新生成 oracle 标签，不能把它当作事后 selection 参数。
...
| 字段 | shape / 逻辑 dtype | 说明 |
|---|---|---|
| `refinement_run_id` | string | checkpoint、固定原阈值规则、同分裁决、输入 manifest 与 schema 的联合身份；也是本 addon 的 `addon_run_id` |
| `source_route` | enum string | `multi_threshold` 或 `f1_baseline` |
...
Selected-final 的每个 `voxel_feat__{source_name}` 固定为 `[K_f,C]`，逐 index 对齐 `final_voxel_index_local_zyx`；不得另造未说明的 voxel 表。它还保存完整 BOX 级 P 表和自己的完整 refined pocket receptor 表，不使用 candidate membership，也不与来源 BOX 特征混用。多个局部组件与原 proposal IoU 同分时，依次以局部概率均值、体素数、最小局部 ZYX 坐标裁决，具体方向与 Stage1 计划一致。proposal/refined IoU、prompt 和 refined pocket 都可派生。精修失败不得静默覆盖原 Global Proposal；下游显式决定是否回退。F1 baseline 即使启用该可选路径，也不因此创建 CLG、selector 或 selection 身份。
...
| `scores` | CLG 路线为 Global Proposal 统计 + 指定 `selector_run_id` 的 score addon；F1 baseline 只有 Global Proposal 统计，不伪造 selector score |
...
| `P_pos/P_prob/P_feat_*` | BOX 级完整 `P_pos_box_xyz/P_probability/P_feat_L2/L3/L4`；所有候选共享 |
...
| **Stage1 selector** | 一个 CLG 的 Group-parent voxel/P/receptor 共享表、Global Proposal membership、rank map 派生量和树关系 | 一个 CLG 是一个样本；输出逐 blob `predicted_max_iou/selection_logit`，在原树上做含空集的结构化反链 DP；F1 baseline 不运行该网络 |
...
- 每个 addon 都能由 `(addon_name,addon_run_id,pdb_id,box_type,box_id)` 唯一寻址，且 `addon_inventory` 的内容哈希与实际压缩产物一致。
- `origin_xyz` 与可能越界的 `box_start_zyx` 按 voxel-center 公式一致；从 BOX 与完整网格交集冷读并补零后 shape 必须恰为 `shape_zyx`，不做 clamp 或平移。
- infered proposal 的完整 mask 严格位于 `BOX ∩ original density grid` 内部；对每个面使用 §4.2.2 的统一边界审查规则。
- 一个 CLG 只有一个 `group_parent_candidate_index`，且该 index 的 `candidate_node_id` 等于 `group_parent_node_id`。
- 所有 candidate voxel/receptor indices 均在 parent 表范围内，同一候选内部唯一且排序；不同候选可交叉。全部候选读取完整 P 表，盘上没有 candidate-P membership。
...
- selector score 的 `candidate_index` 严格为 `0..N-1`，`candidate_node_id/predicted_max_iou/selection_logit` 与基础 CLG 候选数组逐 index 对齐；selection indices 只能索引该数组。
...
- 所有 NPZ 产物能证明由 `np.savez_compressed` 或语义等价的压缩容器写出。
...
5. voxel feature-source 的最终字段全集，以及已固定 `P_feat_L2/L3/L4`、`receptor_feat_L1/L2/L3/L4` 的精确源码出口、通道数与 storage dtype。
6. `infered_box` 是否回填某种 occurrence 锚（默认不填，靠覆盖/距离）。
7. pocket 半径与候选 receptor membership 的确定性参数；实际参数必须进入 manifest。
