...
# transformers4 必须排在 common 之前；官方规范化成功项组成跨 PDB 列表并传 batch_size=100，规范化失败项仍逐条调用公开接口；实际分块与 202 token 截断由冻结源码决定。
...
python_args=(
    "${TASK_PROJECT_ROOT}/Data_Preprocessing/Ori_Data_upgrade/ligand_language_model/encode_smi_ted.py"
    --data-root "${data_root}"
