...
# SMI-TED 可以由单个 A100 任务或零基连续 GPU array 启动。每个数组任务把自己
# 负责的全部 PDB occurrence 作为一个全局列表交给官方 model.encode；官方入口
# 内部继续使用 batch_size=100、202 token 截断和自编码器池化。独立运行时固定
# transformers 4.57.6，以避免新版把普通化学 token 静默映射为 <pad>。
...
model_dir=${weight_root}/models/smi_ted_289m
runtime_dir=${weight_root}/runtime/smi_ted_289m

export PYTHONNOUSERSITE=1
# transformers4 必须排在 common 之前；common 提供 tokenizers、regex 和
# pytorch-fast-transformers，并保留前探时验证过的其他公共依赖。
export PYTHONPATH="${runtime_dir}/transformers4:${runtime_dir}/common${PYTHONPATH:+:${PYTHONPATH}}"
export HF_HUB_OFFLINE=1
export TRANSFORMERS_OFFLINE=1
export TOKENIZERS_PARALLELISM=false

"${python_bin}" \
    "${TASK_PROJECT_ROOT}/Data_Preprocessing/Ori_Data_upgrade/ligand_language_model/encode_smi_ted_289m.py" \
    --data-root "${data_root}" \
