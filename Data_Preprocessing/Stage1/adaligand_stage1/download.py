...
def download_one_pair(
    ...
    targets = [
        ...
        ("meta", EMDB_META_URL.format(emdb_id=emdb_id), root / "raw" / "meta" / f"{pdb_id}.meta.json"),
...
def write_failed_downloads(root: Path, results: list[dict[str, Any]]) -> None:
    """
    汇总写入下载失败记录。
    ...
    输出:
        - None: 若存在失败则写入 `raw/_failed_download.jsonl`
    """
    ...
    if failed:
        write_jsonl(root / "raw" / "_failed_download.jsonl", failed)
    else:
        (root / "raw" / "_failed_download.jsonl").unlink(missing_ok=True)
