...
- 上文“双 CPU96 继续运行”是 Phase-2 前的历史状态。当前补算 v2 已完成 5,984 行 unique：2,984 skipped + 2,986 success + 6 known + 8 unknown，status SHA-256 `3231dfe5…23e`，且因 unknown 未形成 `f_supplement_release`。已 release 的补算 v1 status/release SHA-256 `04d6474e…39a7` / `9a445fa4…9426` 与 Stage E base4 `380844d0…325f` 必须冻结。当前 `316116/318350` 均保留 `after+try`，两节点无 F/Loky/Chimera/MapQ writer；Phase-2 使用用户最新批准的当前-run cap100，只能把正式 Stage F 视图 11→19、补算 v2 视图 6→14，两个新数量在事务、回归和 gate 通过前均不得称为已生效。scratch v4 早已闭合，不得因这个新停点重复执行。
- Phase-2 不能直接复用旧 v2 plan 的历史进度碰撞阈值：正式日志已经越过该阈值，会在 workload 启动前产生错误的 guard=75 成功退出。checkpoint `a397e45` 为本次 v2 增加显式 formal-held 模式，持续绑定 `316116/cnode04` 的 after+try 文件身份与零 writer probe；正式锁或进程状态漂移时使用独立非零 76、回收 `child_pgid_318350` 并跳过 release gate。旧 progress guard 与 75 语义保持不变。该入口仍是 run-specific 运维脚手架，不是新的 F 科学路径；它不带 `--overwrite`，已有合法质量三件套只做完整 validator 后 `skipped`，不会重跑 Chimera、MapQ、CC 或 Q-score。
