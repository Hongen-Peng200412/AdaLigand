...
- 当前 `316116/318350` 均以精确 `after+try` 停写，`316117` 未启动。现有 run-only exclusion 为 5 个，六者全部追加会达到 11 并违反严格 `<10` 上限；且同类聚集属于系统性趋势。因此必须等待用户显式选择祖传数值等价的通用有界内存适配、放宽本 run exclusion 上限或批准另行命名的科学替代，不能由 Agent 自行降级或释放 G。
