...
大网格上的 `cc_all*` 外部工具崩溃默认必须继续记为 unknown；只有用户显式批准且完整绑定证据的 run-scoped manifest 才能把它转为 `known_failed:run_policy_excluded`。不能静默裁图、降采样、改非零 mask、改均值/归约精度、换相关工具或用 contour CC 替代。诊断 harness 必须包含一个已成功 canonical 控制样本，并证明 fresh-process 的两个 ALL 值逐位复现；导入失败或未真正进入 `measure correlation` 的运行只作工程失败证据。若 classic Chimera 的全点物化在单 PDB、单进程下仍 signal 11，任何有界内存适配都必须保持实验图非零点的 C-ravel 顺序、float32 点/权重、原世界变换、原三线性插值与原 `FitMap.overlap_and_correlation` reducer，并先在合成与分层真实控制上证明逐位等价。单纯分块累加 sufficient statistics 会改变浮点归约顺序，不能自动宣称祖传数值等价。
...
补算的历史分层不可改写：Stage E 共享 `exclusions.jsonl` 是 base 来源，正式 Stage F 加法视图和每个补算 run 的加法视图分别拥有自己的 SHA 与 release 身份。已成功 release 的补算 run 不得因后续分片失败而被追加、重放或改写；新失败只能在所属的新 run 和正式 Stage F 视图中以受检事务迁移收口。在新 status/release gate 成功前，迁移目标不能在文档或下游中冒充已生效产物。
...
`reports/runs/{run_id}/exclusions.jsonl` 是单次正式 run 的显式排除清单，不是科学黑名单。每行必须完整给出 `schema_version/pdb_id/run_id/stages/reason/detail/authorization/decision_scope/downstream_policy/evidence`；`decision_scope` 固定为 `current_run_only`，`downstream_policy` 固定为 `exclude_from_training_and_inference`，`stages` 只能覆盖 E/F。命中项在 E/F 写 `known_failed:run_policy_excluded`，并在状态 detail 中引用完整的 manifest/provenance；这类审计 provenance 不等于公开质量产物，也不得据此创建占位 `quality/*.jsonl`、`quality/*.provenance.json` 或 `quality_atoms/*.npz`。样本仍保留在 A 样本宇宙、状态分母与审计中；不删除 `pair_list`，不伪造 success，G 也不会把它写入候选。清单缺字段、跨 run、重复 PDB、非法 stage 或哈希漂移都必须 fail-fast。

正式 run `adaligand_ag_20260711T154658` 曾使用一条严格受限的 run-only 运行策略。旧上限要求累计不超过 9，并在第 10 项或同类聚集前停下询问；Fresh ALL 诊断触发该门后，用户当时把**本 run** 的硬上限放宽到 30，并授权把 `9bw7/9c1k/9dgr/9fkb/9mxv/9nw3` 以 `run_policy_excluded:chimera_full_grid_cc_signal11` 收口。与既有 `8ckb/8glv/9e5c/9fqr/6kgx` 合计 11/22,386（约 0.049138%）。这些样本仍留在状态分母，排除训练、推理和 G 候选，不得伪造 success 或质量三件套。该历史上限、原因和 ID 只存在于绑定本次 run 的 manifest；生产 Python 禁止 PDB allowlist，未来 run 也不得自动继承 30 或这六个 ID。后续 cap100/cap200 决定另有独立证据，不回写旧 manifest。

Phase-2 又冻结了 8 个已取证外部工具边界：`7ju4/7kzm/7z8f/7z8i/8olb` 为 `chimera_full_grid_cc_signal11`，`7yiu/8e45/8j07` 为 `chimera_molmap_timeout_3600s`。后一类的 detail 必须区分 `7yiu/8e45` 的 `missing_ccd_template_fetch` 与 `8j07` 的 `large_grid_and_model`；不得把三者统一伪装成“大图”或笼统人工超时。用户随后把**当前 run** 的 exclusion 上限从 30 提升到 100；旧 30 只作首轮六例迁移的历史证据，Phase-2 使用 100，但仍要求逐例证据，不能自动消费配额。Phase-2 已受检迁移完成：正式 Stage F 视图为 19 条、SHA-256 `f9482353…fea8`，补算 v2 视图为 14 条、SHA-256 `c1755825…6d45`；Stage E base4 与已 release 的补算 v1 不变。这 8 条公开质量产物继续为 0/3，不生成占位 JSON/NPZ。补算 v2 已以 5,984 unique、5,970 skipped + 14 known、零 unknown/duplicate/silent missing 通过 gate，status/release SHA-256 为 `8babca24…d0c` / `123bed2c…fd69`；随后正式 `316116` 在新鲜进程门和独立复核后只解除精确 try-lock，由原 allocation 以 F12/no-overwrite 恢复。合法既有三件套始终只验证并 skipped。

### 受控 unknown 的运行级 gate waiver（仅运维）

默认契约不变：没有显式 waiver 时，任何 `unknown_failed` 都阻断 gate 和 G。若后续当前 run 出现少量、固定 ID、逐例根因已经闭合的 unknown，且目标样本已形成稳定终态、无活动 writer、无兄弟样本/共享文件污染、科学契约不变，公开产物明确 0/N 或完整通过现有 validator，并能安全排除训练/推理/G，则可以使用 **run-scoped controlled-failure waiver** 让本次运行先继续；不得停止仍在正常工作的 producer，只为把状态改名而重放科学计算。

waiver 不是失败重分类。canonical raw status、原始错误和状态分母必须保持不变；gate 与 G 必须显式消费同一份受检 overlay，把精确命中项单列为 `waived_controlled_failures`，不能计入 success，也不能伪造为原生 known。每条至少绑定 `run_id/stage/pdb_id`、status 文件 SHA、canonical raw-row SHA、原始 status/reason/error、attempt/job/node/tool/code 身份、诊断证据 SHA、required-artifact 完整性快照、无活动 writer 快照、用户授权、累计数量与本 run 上限、下游排除策略、`scientific_contract_unchanged=true`、`no_placeholder_artifacts=true` 和退出条件。未匹配、多匹配、SHA/run/stage/cap 漂移、未列 unknown、duplicate、silent missing/extra、非法 schema、部分损坏产物、系统性增长趋势或任何科学不确定性一律 fail-closed；strict smoke 不接受 waiver，其他 stage 也不得未经专门证明自动继承。这里 fail-closed 的含义仅是“未经用户决定不放行”：Agent 必须先冻结现场并报告真实范围、证据、风险、重跑成本与可选路径，不得把修改代码、停止当前 producer 或昂贵重跑当作默认动作。用户可以在知情后选择接受瑕疵顾全全局，或选择严格阻断/修复；两种决定都必须形成 run-scoped 记录。

当前正式 run 的最新用户授权上限是 200；旧上限 30/100 继续作为各自历史决策证据，不回写旧 manifest。200 是硬上限而不是自动配额，每批仍需单独取证和明确授权。生产默认入口必须在无 waiver 参数时保持原有严格行为。后续“补票”是在不打断当前 producer 的前提下，为一般化、无 PDB allowlist 的原生 failure classifier/修复补齐测试和文档，使未来 clean run 不再需要 overlay；补票闭合后，run-specific 命令/ID 脚本按一次性脚手架规则退休，manifest 与 summary 只归档为历史证据。

当前实现使用 `controlled_failure_waiver.py` 的 manifest schema v1；轻量 `stage_f_process_audit_contract.py` 验证 canonical process-audit schema v3、probe/脚本身份、调度快照、节点和零 writer，且不会导入 quality/Chimera/MapQ。`stage_release_gate.py` 与 `g_filter.py` 必须同时显式传入 `--controlled_failure_waiver` 和 `--controlled_failure_waiver_sha256`；任一参数缺失、SHA 漂移或只让一侧消费都会阻断。manifest 顶层绑定授权、cap、pair-list、全部 status 分片、零 writer 进程审计、gate/G 代码身份和退出条件；每条记录再绑定 raw status 行、最终 attempt、诊断证据、分类与 Stage F 三件套 0/3。正式 F gate 要求进程审计不超过 15 分钟，summary 保留原始 `unknown_failed` 计数并另列 `waived_controlled_failures`；G analyze 必须先读取成功 `f_release` 并匹配同一 waiver 路径/SHA/cap/数量，随后可完整重验已变旧的历史进程审计。G 把命中项保持为 raw unknown，只增加 waiver 分类和 manifest SHA，不读取这些 PDB 的质量文件，也不把它们计入 eligible/candidate。strict smoke 与非 Stage F gate 不接受这份 waiver。

2026-07-20 用户明确批准为正式 F 的 45 条 raw unknown 建立同一份 gate/G waiver，不重跑、不伪造 success/known、不生成占位产物。正式 status 仍为 22,386 unique：16,944 skipped、5,323 success、74 known、45 unknown；45 条由 32 个 Chimera ALL signal-11、8 个外部工具 timeout、2 个 fatal-log 和 3 个 MapQ Q 越界组成，公开质量三件套均为 0/3。它们与既有 19 条 `run_policy_excluded` 不重叠，累计受控数量为 64，低于 cap200；两类集合必须在报告中保持分离，不能把 45 条改写成新的 exclusion manifest。
...
- `316116` 首轮推进到 22,363/22,386 后，现场证据将唯一活动工程长尾定位为 `6kgx` 的 post-MapQ occurrence 投影；它没有公开质量三件套。用户授权后，Stage E 共享 manifest 保持 SHA-256 `380844d0…325f`，Stage F 加法视图先以 SHA-256 `3b10abb5…8ee8` 追加 `6kgx`，后因六例稳定 signal-11 run-only 授权迁移为 11 条、SHA-256 `10c5d923…c8ac`。所有命中项保留在状态分母并写 `known_failed:run_policy_excluded`，但不进入训练、推理或 G 候选，也不生成占位质量产物。`316116` 已于 2026-07-17 14:16+08:00 复用受检 run_cmd SHA-256 `6c1f89a5…c00d` 在原 CPU96 allocation 恢复 F12；`316117` 继续依赖等待且只运行 analyze，不自动执行示例阈值或写 `keep_list`。
- 2026-07-15 启动的尾段补算 `318350` 使用独立 run `adaligand_ag_20260711T154658_fsupp96_v1`；v1 尾段 `[19386,22386)` 选择 2,990 个 PDB，plan/ID SHA-256 为 `1d5c1217…dff4f` / `acacde79…cea80`。首轮 353 个 unknown 已完整分解为 289/15/45/4 四类确定性 MapQ/Chimera 适配误拒，并由 commit `751c8b5`、远端 369 项全套、8 样本 shadow/replay、`9jcs` 四 CC 零漂移和 7 样本 promotion 闭合。修复后补算 v1 status SHA-256 `7938aea615c19029265587d556622c8f2d6aea5acff59a94baa85813698496b6`，仅余 `9bw7/9c1k/9dgr/9fkb/9mxv/9nw3` 六个 unknown。
- 六者的 `n_jobs=1` shadow 位于 `/storage/penghongen/AdaLigand/Ori_Data/reports/runs/adaligand_ag_20260711T154658/stage_f_cc_sigsegv_shadow_20260717_v1/`，step `318350.61` 仍为 6/6 fail，status/acceptance SHA-256 为 `5c41167deb1e17e8fade06c588c25bf4786048ca9286f4e1e317b4c296af50f9` / `67cc083c94efbb72648db07e5d62ec8fee195bbf3a3dc59eecb5dcbf0ed868b2`。ALL-only v1 step `318350.63` 因 harness `ImportError` 而零科学计算；修正后的 v2 目录 `stage_f_cc_all_fresh_process_shadow_20260717_v2/` 由 step `318350.64 COMPLETED 0:0` 在 `00:04:24` 闭合，`9hhl` 的两个 ALL 值与 canonical 逐位相同，`9fkb` fresh 子进程仍 signal 11，acceptance SHA-256 为 `e0bef08c6a4da804d0173ccc920e3e58316912ee4fea303bf9045ad4f50032e9`。运行节点约 2 TB 且无 OOM 证据，不能把故障简化为宿主 RAM 不足。
- 用户已选择只对本 run 排除六者，并把本 run 上限放宽到 30；累计为 11/22,386（约 0.049138%）。该决定不改变四 CC 或通用科学契约。专用工程由 `879f2be/aec286b/ebbef37` 闭合，transition/supplement resume/formal resume/test SHA-256 分别为 `55e81acc…a3dfb` / `3da2b388…fc43` / `f8adf31f…6a95` / `eed82818…b335`；本地专项 18 passed、全套 377 passed+10 skipped，远端 Linux 全套 387 passed。新鲜跨节点进程门与原子迁移后，正式 Stage F 视图为 11 条、SHA-256 `10c5d923…c8ac`，补算 v1/v2 六条清单 SHA-256 为 `6f3a0a88…7f35` / `43da55a7…7d40`，正式共享 4 条清单仍为 `380844d0…325f`。六例质量三件套继续保持 0/3：不创建占位 JSON/NPZ，后续用产物完整性检查自然排除。补算 v1 已以 2,990 unique、2,984 skipped + 6 known 和零 unknown/duplicate/silent missing 通过 gate，status/release SHA-256 为 `04d6474e…39a7` / `9a445fa4…9426`；wrapper 已进入 5,984-ID 的独立 v2，IDs/plan SHA-256 为 `7f427993…17c6c` / `116084c3…64fc`。v2 guard/child PGID/F12 门闭合后，正式 `316116` 也已受检恢复；生产路径不得出现六 ID allowlist。
- 唯一有效的硬中断恢复证据位于 `/storage/penghongen/AdaLigand/Ori_Data/reports/runs/adaligand_ag_20260711T154658/stage_f_scratch_recovery_20260716_v4/`。v4 inventory 为 139,940 条文件行和 11,445 个 attempt，两个清单 SHA-256 为 `e5f3075c…c502a` / `3f9f2096…a863`。双新鲜进程门、零删除 audit、独立 bundle 验收与 journal/fsync apply 已闭合：manifest 恰回收 4,341 个 transient，923 个受影响 attempt 的 10,260 个普通 nontransient 与 2,568 个公开质量三件套路径通过冻结身份复核，预计回收约 958.01 GiB；v1/v3 永久作废且未被 apply。此前按顺序删除 try-lock 并恢复，只是 scratch 事故闭合的历史步骤；后续 adapter recovery 与 Fresh ALL 隔离诊断生成的新 try-lock 也已按各自受检门精确解除。当前 `316116` 与 `318350` 均在各自原 CPU96 allocation 继续 F12，不能把历史锁解释为 scratch audit/apply 尚未闭合，也不得重复执行 v4。
- 上文“双 CPU96 继续运行”是 Phase-2 前的历史状态。补算 v2 首轮曾完成 5,984 行 unique：2,984 skipped + 2,986 success + 6 known + 8 unknown，status SHA-256 `3231dfe5…23e`，且因 unknown 未形成 `f_supplement_release`。已 release 的补算 v1 status/release SHA-256 `04d6474e…39a7` / `9a445fa4…9426` 与 Stage E base4 `380844d0…325f` 始终冻结。Phase-2 完成 formal 11→19 与 v2 6→14 的原子迁移后，补算 v2 重放最终以 5,984 unique、5,970 skipped + 14 known 和零 unknown/duplicate/silent missing 通过 gate；正式 `316116/cnode04` 已于 2026-07-18 21:53:48+08 保留 after-lock、只删除精确 try-lock 后在原 CPU96 allocation 恢复 F12/no-overwrite。fresh process audit/intent/result SHA-256 为 `eb6f8638…ac6f6` / `8d0caa57…2ee6` / `bc5006a2…60d2`，`318350` 不再运行且不得重复放行。scratch v4 早已闭合，不得因这个新阶段重复执行。
- Phase-2 不能直接复用旧 v2 plan 的历史进度碰撞阈值：正式日志已经越过该阈值，会在 workload 启动前产生错误的 guard=75 成功退出。checkpoint `a397e45` 为本次 v2 增加显式 formal-held 模式，持续绑定 `316116/cnode04` 的 after+try 文件身份与零 writer probe；正式锁或进程状态漂移时使用独立非零 76、回收 `child_pgid_318350` 并跳过 release gate。旧 progress guard 与 75 语义保持不变。该入口仍是 run-specific 运维脚手架，不是新的 F 科学路径；它不带 `--overwrite`，已有合法质量三件套只做完整 validator 后 `skipped`，不会重跑 Chimera、MapQ、CC 或 Q-score。当前受控 unknown waiver 规则不追溯替代已经闭合的 Phase-2，也未修改运行中的补算命令。
- 正式 `316116` 已于 2026-07-20 12:27:46+08:00 `COMPLETED 0:0`；raw F 四终态保持 16,944 skipped、5,323 success、74 known、45 unknown，`f_release` 成功绑定同一 waiver SHA-256 `805f4c49…0a04`，没有重启 F producer，也没有把 45 条写进 exclusion manifest 或质量目录。
- 正式 `316117` 已于 2026-07-20 18:47:51+08:00 `COMPLETED 0:0`，且只运行 analyze。G 状态为 22,251 success、90 known、45 raw unknown，生成 662,078 条 `candidates.pending` occurrence；候选/分布 SHA-256 为 `86c9cfc7…786e` / `9bf8e7cf…c7dd`。独立审计 job `321031` 重算主键集合、known/waived/eligible、候选排序唯一性与全部分位数并通过，summary SHA-256 `3a2fa87e…9d00`；45 条 waiver 仍为 0/3，`keep_list.jsonl` 不存在。两个同时属于上游 known 与 F waiver 的 PDB（`1zku/6r8n`）在互斥 G 状态中按 waiver-first 记 raw unknown，分布头仍分别报告两个可重叠集合。
- G 的唯一 map-level schema v2 算法已经锁定；最终分辨率、selected CC、配体 Q、口袋 Q 和合格比例数值仍按“先看正式分布再显式配置”。本次 DAG 没有消费示例配置，也没有冒充最终科学筛选或写 `keep_list`；后续 filter 是用户显式授权的独立任务。
