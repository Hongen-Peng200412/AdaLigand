...
当前用户授权上限 100 是硬上限而不是自动配额，每批仍需单独取证。生产默认入口必须在无 waiver 参数时保持原有严格行为。后续“补票”是在不打断当前 producer 的前提下，为一般化、无 PDB allowlist 的原生 failure classifier/修复补齐测试和文档，使未来 clean run 不再需要 overlay；补票闭合后，run-specific 命令/ID 脚本按一次性脚手架规则退休，manifest 与 summary 只归档为历史证据。
