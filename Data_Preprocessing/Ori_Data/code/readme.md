...
当前正式 run `adaligand_ag_20260711T154658` 另有一条严格受限的 run-only 运行策略。旧上限要求累计不超过 9，并在第 10 项或同类聚集前停下询问；Fresh ALL 诊断触发该门后，用户显式把**本 run** 的硬上限放宽到 30，并授权把 `9bw7/9c1k/9dgr/9fkb/9mxv/9nw3` 以 `run_policy_excluded:chimera_full_grid_cc_signal11` 收口。与既有 `8ckb/8glv/9e5c/9fqr/6kgx` 合计 11/22,386（约 0.049138%）。这些样本仍留在状态分母，排除训练、推理和 G 候选，不得伪造 success 或质量三件套。该上限、原因和 ID 只存在于绑定本次 run 的 manifest；生产 Python 禁止 PDB allowlist，未来 run 也不得自动继承 30 或这六个 ID。manifest 必须验证 run/stage、授权、证据、下游策略和 SHA，任何漂移 fail-closed；其退出条件是本 run 的 F release、G analyze 与最终 A–G 审计闭合，专用生成/恢复脚手架须在最终维护收口中审查并归档或移出生产入口。
...
- 正式 `316116` 的科学 producer 已完成 22,386/22,386，并在原严格 gate 处因 45 条 raw unknown 进入精确 try-lock；`f_release` 尚未形成，`316117` 仍等待依赖。用户已批准 cap200 和 45 条逐行受控 waiver；本地默认严格实现、gate/G 共用路径与回归已完成，下一步是冻结服务器 manifest、远端全套测试和 gate-only 原地放行。该放行不得重启 F producer，也不得把 45 条写进 `exclusions.stage_f.jsonl` 或质量目录。
