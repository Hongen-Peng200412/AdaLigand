...
- 用户已选择只对本 run 排除六者，并把本 run 上限放宽到 30；累计为 11/22,386（约 0.049138%）。该决定不改变四 CC 或通用科学契约。专用工程检查点 `879f2be` 只新增 manifest transition、正式/补算恢复脚本和回归测试；transition/formal resume/supplement resume/test SHA-256 分别为 `8e2e1346…ba489` / `6e16aec8…acc79` / `99f81c76…ee787` / `46d07664…3ebdc`，专项 12 passed、相关 42 passed/2 Linux-only skipped。本机因缺 `rdkit/gemmi` 的 16 个 collection errors 不算全套；远端 Linux 全套、manifest 实例 SHA/行数和受检锁恢复闭合前，`316116/318350` 继续保持精确 `after+try`，`316117` 不启动。生产路径不得出现六 ID allowlist。
