# Stage C 受体 token、原子坐标和身份映射。
# 主要输入：解析后的受体原子 XYZ Å、元素/残基信息和化学键类型。
# 主要输出：receptor token 坐标(N,3)、键表、局部密度和 feat(N,49)。
# 关键边界：合法 TRIPLE 键使用兼容值 6；特征行必须与 token/坐标顺序一致。
...
from adaligand_preprocessing.stages.stage_c.ligand_objects import get_ccd_mol
