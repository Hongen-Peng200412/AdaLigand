...
2. `held_out_pipeline/redundancy.py`：先看 MMseqs2 命令参数，再看 entity 命中怎样进入容量图和三个一对一匹配，最后看 `or/and` 如何只组合 chain/residue 两级判断。
3. `held_out_pipeline/selection.py`：看参考冗余排除、内部冲突图、固定种子贪心极大独立集 `full_test`、至多 200 项 `test_0` 抽样和身份证回填。
