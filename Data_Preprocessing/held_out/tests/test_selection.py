"""验证 held-out 冲突图极大独立集, 统一身份证与测试视图子集关系."""
...
def test_greedy_selection_is_maximal_without_connected_component_collapse() -> None:
    """A-B 与 B-C 冲突不产生 A-C 冲突, 输出为有直接拒绝见证的极大独立集."""
...
def test_finalize_builds_maximal_full_test_and_nested_views(
    ...
    eligible_set = {
        ...
        if identity["selection"]["base_eligible"]
    }
    # set[frozenset[str]], 合成 held-out 内部冗余边的无向端点集合.
    conflict_pairs = {frozenset((edge["pdb_A"], edge["pdb_B"])) for edge in edges}

    assert all(not (pair <= full_test_set) for pair in conflict_pairs)
    assert all(
        any(pdb_id in pair and bool(pair & full_test_set) for pair in conflict_pairs)
    ...
    assert full_test_view["occurrence_filter"] is None
    assert test_0_view["parent"] == "full_test"
    assert test_1_view["parent"] == "test_0"
    assert all(
