"""把 held-out 基础事实和共享 PDB 边证据合成为参数化身份视图.

主要入口是 :func:`finalize_identity_views`. 该入口只对共享边证据应用指定 coverage 模式与阈值, 再排除参考集冗余, 最后把 held-out 内部冲突图的固定种子贪心极大独立集写为 `full_test`, 并派生 `test_0`, `test_1`.

本模块不解析 mmCIF, 不运行 MMseqs2, 也不改变质量, 资产或序列阈值.

共享输入位于 `shared_output_root`, 当前参数的全部产物位于 `split_output_root`. `redundancy_edges.jsonl` 一行对应一个已应用当前参数的 PDB 对; `held_out_identity.jsonl` 一行对应一个冻结 held-out PDB; `full_test.json`, `test_0.json`, `test_1.json` 各保存一个身份视图; `summary.json` 与 `_COMPLETE` 记录本次 split 正常写完的事实.
"""
...
from held_out_pipeline.redundancy import classify_pdb_redundancy
...
def finalize_identity_views(
    ...
    """从共享 PDB 边证据生成一个参数化 split 的身份证和测试视图.

    形状符号:
        - N_edge: 共享证据中至少含一条高重复 entity 命中的 PDB 对数.
        - N_held_out: 冻结 held-out PDB 数.
        - N_full_test: 贪心极大独立集中的 PDB 数.
        - N_test0: test_0 中的 PDB 数, 等于 min(test_0_size, N_full_test).
        - N_test1: test_0 中 occurrence 总数严格位于 `(1, 100)` 的 PDB 数.

    输入参数:
        - shared_output_root: Path, held_out_base, PDB 边证据和边规模汇总的共享根目录.
        - split_output_root: Path, 当前 mode/threshold 的全部参数相关产物目录.
        - held_out_pdb_path: Path, 冻结 held-out PDB JSON 列表.
        - mode: str, 取 chain, residue, or 或 and.
    ...
    返回字段:
        - summary: dict, 保留共享 edge summary 并增加当前判定和最终视图规模.
            - summary.raw_qualifying_alignment_count: int, 通过类别 identity 与双向 0.80 coverage 的原始 MMseqs2 行数.
    ...
    落盘产物:
        - `redundancy_edges.jsonl`: JSONL; 每行是共享证据加当前参数与判定字段.
            ...
            - redundant: bool, 当前 mode 选取或组合 chain_pass 与 residue_pass 的结果.
        - `held_out_identity.jsonl`: JSONL; 每行一个冻结 held-out PDB 的统一身份证.
            ...
            - redundancy: dict, 当前 PDB 的参考和 held-out 内部冗余摘要.
                - redundancy.pdb_coverage_mode: str, 当前 chain/residue/or/and 模式.
        ...
        - `full_test.json`: dict; 满足全部前置条件且 held-out 内部无冗余边的贪心极大独立集.
            - schema_version: int, 当前为 1.
            - pdb_coverage_mode: str, 当前 chain/residue/or/and 模式.
            - pdb_coverage_threshold: float, 当前 PDB coverage 包含边界.
    ...
    for evidence in edge_evidence:
        ...
        decision = classify_pdb_redundancy(
            ...
            float(evidence["chain_B"]),
            float(evidence["residue_A"]),
            float(evidence["residue_B"]),
            mode,
            threshold,
        )
        edges.append(
    ...
    _write_jsonl(split_output_root / "redundancy_edges.jsonl", edges)
    # dict, 与参数无关的 alignment, entity hit 和 PDB edge evidence 规模.
    ...
    _write_jsonl(split_output_root / "held_out_identity.jsonl", identities)
    ...
    _write_json(
        split_output_root / "full_test.json",
        {
            **common_view_fields,
            "name": "full_test",
    ...
    _write_json(split_output_root / "summary.json", summary)
    _write_text(split_output_root / "_COMPLETE", "")
    return summary
